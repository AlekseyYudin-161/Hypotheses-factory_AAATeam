"""pgvector: подключение + запись. register_vector для передачи вектора списком float."""
...
def upsert_document(cur, doi, title=None, pub_year=None, source=None, ner_tag=None):
    cur.execute(
        """INSERT INTO documents (doi, title, pub_year, source, ner_tag)
           VALUES (%s, %s, %s, %s, %s)
           ON CONFLICT (doi) DO UPDATE SET title=EXCLUDED.title, source=EXCLUDED.source""",
        (doi, title, pub_year, source, ner_tag),
    )

def insert_chunk(cur, doi, chunk_text, embedding):
    cur.execute(
        """INSERT INTO knowledge_chunks (doi, chunk_text, embedding, text_search)
           VALUES (%s, %s, %s, to_tsvector('russian', %s)) RETURNING chunk_id""",
