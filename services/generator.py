...
from services.documents import documents_for_backend
...
def _local_generate(
    ...
    uploaded_documents = [document for document in (documents or []) if document.get("text")]
    document_names = ", ".join(document.get("name", "") for document in uploaded_documents)
    ...
    seed_payload = json.dumps(
        {
            ...
            "documents": [document.get("sha256", document.get("name", "")) for document in (documents or [])],
        },
        ensure_ascii=False,
        sort_keys=True,
    )
    ...
    for index, raw in enumerate(base_templates[:count], start=1):
        ...
        if uploaded_documents:
            document = uploaded_documents[(index - 1) % len(uploaded_documents)]
            evidence.append(
                {
                    "triplet": item["chain"][0] + " → " + item["chain"][1] + " → " + item["chain"][-1],
                    "source": document.get("name", "Uploaded document"),
                    "source_url": "",
                    "local_path": document.get("local_path", ""),
                    "page": 1 if document.get("page_count") else None,
                    "chunk_id": f"UPLOAD-{index:02d}-01",
                    "evidence_fragment": document.get("excerpt", "") or document.get("text", "")[:500],
                    "confidence": round(rng.uniform(0.68, 0.86), 2),
                    "support_type": "uploaded_document",
                    "knowledge_base": "uploaded_documents",
                }
            )
        ...
        if document_names:
            document_note = {
                "ru": f" При формировании учтены загруженные документы: {document_names}.",
                "en": f" Uploaded documents considered: {document_names}.",
                "zh": f" 已考虑上传的文档：{document_names}。",
            }[lang]
        ...
        result.append(
            {
                ...
                "rationale": item["rationale"] + document_note,
                "mechanism": item["mechanism"],
                "kpi_link": {
...
def _normalize_backend_response(data: Any) -> list[dict[str, Any]]:
    ...
    for item in data:
        ...
        for source in item.setdefault("evidence", []):
            if not source.get("source_url") and not source.get("local_path"):
                raise ValueError("Each source must contain source_url or local_path.")
...
def generate_hypotheses(
    ...
    selected_knowledge_bases = KNOWLEDGE_BASE_CODES.copy()
    if not BACKEND_URL:
        return _local_generate(kpi, constraints, language, selected_knowledge_bases, documents)
    ...
    payload = {
        ...
        "documents": documents_for_backend(documents or []),
    }
